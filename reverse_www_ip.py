...
def get_domain_from_db():
    ...
    # sql = 'SELECT domain  FROM domain_ip_mx WHERE www_ip IS NULL'
    sql = 'SELECT domain  FROM domain_ip_mx WHERE www_ip=""'
    db.query(sql)
...
def get_svr_ip():
    ...
    while 1:
        print queue.qsize()
        domain = queue.get()
        ips,cnames = resolve_domain_record('www.'+domain)
...
def update_data():
    ...
    """
    db = MySQL(SOURCE_CONFIG)
    sql = 'UPDATE domain_ip_mx SET www_ip="%s",www_cname="%s",www_country="%s",www_province="%s",www_city="%s",www_oper="%s" WHERE domain="%s" '
